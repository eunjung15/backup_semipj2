...
$(function () {
  ...
  if (
    ...
    if ($(".product_date").length > 0) {
      ...
      $(".btn_date")
        .find(".date_view b")
        .html(
          xReturnNumber(sel_date[1]) +
            "." +
            xReturnNumber(sel_date[2]) +
            " ~ " +
            xReturnNumber(sel_date2[1]) +
            "." +
            xReturnNumber(sel_date2[2])
        );
      $(".btn_date")
        .find(".date_view em")
        .html("&nbsp;·&nbsp;" + diff_day + "박");
  ...
  $(".product_date").on("daterangepickerclose", function (event, data) {
    // 선택완료
    $(".bg_datepicker").hide();
...
function dateopen_trigger() {
  // Datepicker 오픈 트리거
  var title = "일정 선택";
  $(".comiseo-daterangepicker-triggerbutton").trigger("click");
  var html_content =
    '<div class="bg_datepicker"><strong class="title">' +
    title +
    "</strong></div>";
