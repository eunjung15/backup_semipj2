    ...
    $("#poduct_list_area li a[data-alat]").each(function () {
      if (
        ...
        if (overlap.indexOf(ano) == -1) {
          ...
          if (distance.length > 1) f = Math.pow(10, distance[1].length);
          ...
          if (win_w < 1024) {
            ...
            item[0]
              .find(".cate")
              .html(get_category_name($(this).data("adcno")));
...
function pop_map_m() {
  ...
  var html = $(
    '<li><a href="#"><p class="pic">&nbsp;</p><span class="cate"></span>' +
        ...
        return function () {
          if (win_w < 1024) {
            ...
            if (on_maker !== undefined) {
              on_maker.setImage(
                locations[i][3] == 1 ? get_maker("green") : get_maker("grey")
              );
  ...
  function displayAddress(_latlng) {
    ...
    $.xResponse("/product/srp_search", {
      ...
      if (
        _result.data == null ||
        _result.data.total_count == 0 ||
        _result.data.list.length == 0
      ) {
  ...
  daum.maps.event.addListener(map, "bounds_changed", function (event) {
    if (locations.length < 2) this.setLevel(4);
  });

  $(".pop_m_room .owl-carousel").owlCarousel({
    // Swipe
    stagePadding: 16,
    nav: false,
    center: true,
    ...
    onDragged: function (property) {
      ...
      if (on_maker !== undefined) {
        on_maker.setImage(
          locations[current][3] == 1 ? get_maker("green") : get_maker("grey")
        );
        on_maker.setZIndex(5);
      }
...
function pop_map_pc() {
  ...
  var html = $(
    '<div class="map_info"><a href="#"><div class="map_pic" style="background-image:url(' +
      bg_src +
      ')">&nbsp;</div>' +
...
function select_map() {
  ...
  // 주소검색 취소 클릭시
  $(".pop_map .srp_srch_bar .btn_cancel").click(function () {
    // 취소버튼 클릭시
...
$(function () {
  ...
  $(window).resize(function () {
    ...
    if ($(".mobile .filter_wrap").css("display") === "block") {
      // 상세조건 팝업 자동닫기
      filter_close();
    }
  });
