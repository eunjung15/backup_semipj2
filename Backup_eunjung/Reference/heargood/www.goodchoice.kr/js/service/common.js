...
$.reserveDetail_guest = function () {
  ...
  if (regPhone.test(uphone) == false) {
    ...
    $("form[name=reserveDetail_guest] input[name=uphone]")
      .parent()
      .addClass("inp_error");
    $("form[name=reserveDetail_guest] input[name=uphone]")
      .parent()
      .find("span")
      .html("휴대폰번호를 올바르게 입력해주세요.");
    return false;
  }

  $("form[name=reserveDetail_guest] input[name=uphone]")
    .parent()
    .removeClass("inp_error");
  $("form[name=reserveDetail_guest] input[name=uphone]")
    .parent()
    .find("span")
    .hide();
  ...
  if (regDono.test(do_no) == false) {
    ...
    $("form[name=reserveDetail_guest] input[name=do_no]")
      .parent()
      .addClass("inp_error");
    $("form[name=reserveDetail_guest] input[name=do_no]")
      .parent()
      .find("span")
      .html("예약번호를 올바르게 입력해주세요.");
    return false;
  }
    ...
    if ($(".pc .srch_bar").css("display") == "block") {
      if ($("#keyword").val().length > 0)
        window.location.href =
          "/product/result?keyword=" + encodeURI($("#keyword").val());
  ...
  $.xResponse("/product/get_recommend_non").done(function (result) {
    if (result.count > 0) {
      ...
      $.each(result.data, function (k, recommend) {
        items.push(
          '<li><a href="/product/result?keyword=' +
            recommend +
            '">' +
            recommend +
            "</a></li>"
  ...
  $("#keyword").keydown(function (key) {
    if (key.keyCode == 13) {
      if ($(this).val().length > 0)
        window.location.href =
          "/product/result?keyword=" + encodeURI($(this).val());
      else alert_Msg("검색어가 없습니다.  <br>검색어를 입력하세요.");
    }
  });
...
function pic_resize() {
  ...
  $(".list_4.adcno2 .pic img,.list_4.adcno1 .pic img").each(function () {
    // 해당리스트 가운데 정렬
...
function get_no_total_pagination(settings, notices, el, scroll) {
  if (settings === undefined || Object.keys(settings).length == 0) return false;
...
function spinnerText(msg) {
  ...
  var content =
    '<div class="spinner_text"><div><div class="spinner_w"><span></span></div><p>' +
    text +
    "</p></div></div>";
...
$(function () {
  ...
  $(".pop_guest_reserve_open").click(function () {
    // 비회원 예약내역 조회
    pop_layer_w.call(this, "pop_guest_reserve");
    return false;
  });

  $(".pop_mem_reserve_open, .mobile_reserve").click(function () {
    // 회원 예약내역 조회
    pop_layer_w.call(this, "pop_mem_reserve");
    return false;
  });
  ...
  $(".pop_logout_open").click(function () {
    // 로그아웃
    pop_twobtn(
      "ell",
      "로그아웃 하시겠습니까?",
