...
<%
	...
	try{
		vo = reviewService.showreviewByReviewNo(Integer.parseInt(reviewNo));
...
%>
	...
	<div class="box">
		<form name="frm" method="post" action="reviewEdit_ok.jsp"
			...
			<div class="mb-3">
				...
				<input type = "hidden" name="hotelNo" value ="<%= vo.getHotelNo()%>">
				<input type = "hidden" name = "accNo" value = "<%=vo.getAccNo()%>">
				<input type = "hidden" name = "room_type" value = "<%= vo.getRoom_type()%>"><br>
			...
			<div class="wrap">
					<input type="hidden" name="rate" id="rate" value="<%= vo.getScore() %>" />

					<div class="review_rating">
						<div class="warning_msg">별점을 선택해 주세요.</div>
						<div class="rating">
							<!-- 해당 별점을 클릭하면 해당 별과 그 왼쪽의 모든 별의 체크박스에 checked 적용 -->
						<%	for(int i=0; i<vo.getScore(); i++){ %>
							<input type="checkbox" name="score" id="rating1" value="<%=i %>"
								class="rate_radio" title="1점" checked> <label for="rating1"></label>
						<%} 
							for(int i=0; i<(5-vo.getScore()); i++){%>
							<input type="checkbox" name="score" id="rating1" value="<%=i %>"
			...
			<div class="mb-6">
				...
				<textarea class="form-control" id="exampleFormControlTextarea1"
					name="content" rows="30" ><%=vo.getContent() %></textarea>
			</div>

			<div class="mb-5">
				<input type="submit" id="btn1" value="수정" onclick=submitContents()>
				<input type="button" id="btn2" value="삭제" onclick="location.href='reviewDelete.jsp?reviewNo=<%=vo.getReviewNo()%>'">
			</div>
		</form>
	</div>
</body>
