...
public class reservation_detailDAO {
   ...
   public int insertReservation_detail(reservation_detailVO vo) throws SQLException {
      ...
      try {
         ...
         //3. ps
         String sql="insert into reservation_detail(reCode, room_type, people_num, check_In_Date, check_Out_Date, hotelNo)"
   ...
   public List<reservation_detailVO> selectAll(String keyword, String condition) throws SQLException{
      ...
      try {
         ...
         while(rs.next()) {
            ...
            =new reservation_detailVO(reDetailNo, reCode, room_type, people_num, check_In_Date, check_Out_Date, hotelNo);
   ...
   public int updateReservation_detail(reservation_detailVO vo) throws SQLException {
      ...
      try {
         ...
         String sql = "update reservation_detail"
               + " set reCode = ?  room_type =?, people_num = ?, check_In_Date = ?, check_Out_Date = ?, hotelNo = ?"
               + " where reDetailNo = ?";
         ps = con.prepareStatement(sql);
   ...
   public reservation_detailVO selectRecode(int hotelNo) throws SQLException {
      ...
      reservation_detailVO vo = new reservation_detailVO();
      try {
         con = pool.getConnection();
         
         String sql = "select recode from reservation_detail" + " where hotelNo = ?";
         ps = con.prepareStatement(sql);
